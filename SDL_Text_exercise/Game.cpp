	...
	if (SDL_PollEvent(&event)) {
		...
		case SDL_MOUSEBUTTONDOWN: {
			...
			if (event.button.button == SDL_BUTTON_LEFT) {
				SDL_GetMouseState(&msx, &msy); // Where the mouse clicked cordinates ex(153:53)
				std::cout << msx << ":" << msy << std::endl; // Print the cordinates
...
bool Game::ttf_init()
{
	...
	tempSurfaceText = TTF_RenderText_Blended(font2, "Goodbye World", { 0, 255, 255, 255 });
	textTextureFont2 = SDL_CreateTextureFromSurface(renderer, tempSurfaceText);

	tempSurfaceText = TTF_RenderText_Blended_Wrapped(font1,
		"Hello World!\nThis wraps the text.", { 0, 0, 255, 255 }, 300); // Wrapped Минава не следващия ред ако текста е много дълъг 
	textTextureFont1Wrapped = SDL_CreateTextureFromSurface(renderer, tempSurfaceText);

	tempSurfaceText = TTF_RenderText_Blended_Wrapped(font2,
		"Goodbye World!\nThis wraps the text.", { 0, 255, 255, 255 }, 500); // Wrapped Минава не следващия ред ако текста е много дълъг 
